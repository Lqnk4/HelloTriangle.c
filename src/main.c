...
bool isComplete(QueueFamilyIndices indices) {
  return indices.graphicsFamily.is_present && indices.presentFamily.is_present;
}
...
bool checkRequiredGLFWExtensions(const uint32_t glfwExtensionCount,
  ...
  for (size_t i = 0; i < glfwExtensionCount; ++i) {
    ...
    for (size_t j = 0; j < vkExtensionCount; ++j) {
      if (strcmp(glfwExtensions[i], vkExtensionProperties[j].extensionName) ==
          0) {
        extensionFound = true;
        break;
      };
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
    if (vkCreateSemaphore(device, &semaphoreInfo, NULL,
        ...
        vkCreateFence(device, &fenceInfo, NULL, &inFlightFences[i]) !=
            VK_SUCCESS) {
      perror("ERROR: failed to create synchronization objects for a frame\n");
      return -1;
    }
  }
...
static char *readFile(const char *fileName, size_t *fileSize) {
  ...
  if (dict == NULL) {
    fprintf(stderr, "ERROR: failed to allocate memory while reading file %s\n",
            fileName);
    fclose(fp);
    return NULL;
  }
